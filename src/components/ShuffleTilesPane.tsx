...
export default function ShuffleTilesPane({ src }: Props) {
  const [piece, setPiece] = useState(30) // px, smaller => harder (start hardest)
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <Shuffle className="h-4 w-4" /> 重新打亂
          </button>
          <button
            type="button"
            onClick={() => {
      ...
      <div className="rounded-xl border border-white/10 bg-black/30 p-3">
        {src && board ? (
          <div className="mx-auto overflow-hidden rounded-lg">
            <div
              className="relative mx-auto overflow-hidden rounded-lg border border-white/10 bg-black/30"
              style={{
                width: fitWidth,
                height: 420,
              }}
      ...
      <div className="mt-3 rounded-xl border border-white/10 bg-black/20 p-3">
        <div className="mb-2 flex items-center justify-between">
          <div className="text-[11px] font-semibold text-white/60">方塊大小（越小越難）</div>
          <div className="text-[11px] font-mono text-white/70">{Math.round(piece)}px</div>
        </div>
        <input
          type="range"
          min={30}
          max={260}
          value={Math.round(piece)}
          onChange={(e) => setPiece(parseInt(e.target.value, 10))}
          className="w-full"
          disabled={!src}
        />
      ...
      <div className="mt-3 rounded-xl border border-white/10 bg-black/20 p-3">
        <div className="mb-2 flex items-center justify-between">
          <div className="text-[11px] font-semibold text-white/60">畫布寬度（固定顯示範圍）</div>
          <div className="text-[11px] font-mono text-white/70">{fitWidth}px</div>
        </div>
        <input
          type="range"
          min={420}
          ...
          onChange={(e) => setFitWidth(parseInt(e.target.value, 10))}
          className="w-full"
          disabled={!src}
        />
        <div className="mt-1 text-[11px] text-white/45">
          這會等比例縮放整個打亂拼圖，避免投影時畫面大小一直變。
        </div>
      </div>

      <div className="mt-2 text-[11px] leading-relaxed text-white/45">
        小技巧：把方塊調大比較像「提示」，調小就會變成高難度猜圖。
      </div>
    </div>
  )
}
