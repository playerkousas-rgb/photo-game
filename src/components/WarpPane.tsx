...
function buildSvgFilter(id: string, intensity: number, seed: number) {
  // Strong painterly / wavy distortion using turbulence + displacement.
    ...
    <svg className="absolute h-0 w-0" aria-hidden="true" focusable="false">
      <filter id={id} x="-20%" y="-20%" width="140%" height="140%" colorInterpolationFilters="sRGB">
        ...
        <feColorMatrix
          type="matrix"
          values={
            // mild hue-ish shift approximation
            `1 0 0 0 0\n0 ${contrast} 0 0 0\n0 0 ${contrast} 0 0\n0 0 0 1 0`
          }
        />
      </filter>
      <style>{`
        /* extra styling via CSS filter on element */
        [data-warp="${id}"]{filter:url(#${id}) ${buildFilter(t)} hue-rotate(${hue}deg) saturate(${sat});}
      `}</style>
    </svg>
  )
}

export default function WarpPane({ src }: Props) {
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold text-white/80 hover:bg-white/10 disabled:opacity-50"
          >
            <Shuffle className="h-4 w-4" /> 換一種扭曲
          </button>
        </div>
      </div>
          ...
          <div className="absolute inset-0 grid place-items-center p-6">
            ...
            <div style={warpStyle} className="max-h-full max-w-full" data-warp={filterId}>
              <img
                src={src}
                alt="warp"
                className="max-h-[54vh] max-w-[92vw] select-none"
                draggable={false}
                style={{
                  mixBlendMode: 'normal',
                  filter:
                    tint === 'none'
                      ? 'none'
                      : tint === 'warm'
                        ? 'sepia(0.55) saturate(1.25)'
                        : tint === 'cool'
                          ? 'hue-rotate(190deg) saturate(1.05)'
      ...
      <div className="mt-3 rounded-xl border border-white/10 bg-black/20 p-3">
        ...
        <div className="flex flex-wrap gap-2">
          {(
            [
              { key: 'none' as const, label: '原色' },
              { key: 'warm' as const, label: '暖色' },
              { key: 'cool' as const, label: '冷色' },
              { key: 'mono' as const, label: '黑白' },
            ] as const
          ).map((o) => (
            <button
              key={o.key}
              type="button"
              onClick={() => setTint(o.key)}
              className={
